...
export const Column = ({ listId, listName, cards, index }: Props) => {
  ...
  const handleChangeListName = (name: string): void => {
    socket.emit(ListEvent.RENAME, {name, listId });
  };

  const handleCreateNewCard = (name:string) => {
      ...
      {(provided: DraggableProvided, snapshot: DraggableStateSnapshot) => (
        ...
        >
          ...
          <CardsList
            ...
            cards={cards}
          />
          <Footer onCreateCard={handleCreateNewCard} />
        </Container>
      )}
    </Draggable>
  );
